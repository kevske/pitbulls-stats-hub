...
type LeagueStats = {
    ...
    pointsLeader: [string, TeamStats] | null;
    defenseLeader: [string, TeamStats] | null;
};

const calculateLeagueStats = (allGames: Game[]): LeagueStats | null => {
...
const getLeagueComparisonInfo = (
    ...
    try {
        ...
        const { teamStats, avgPointsFor, avgPointsAgainst, pointsLeader, defenseLeader } = leagueStats;
        ...
        if (pointsLeader && opponentTeamId === pointsLeader[0]) {
        ...
        if (defenseLeader && opponentTeamId === defenseLeader[0]) {
    ...
    useEffect(() => {
        const fetchGamesAndDangerousPlayers = async () => {
            try {
                ...
                // Fetch all games with scores to calculate team statistics ONCE
                const { data: allGames, error: historyError } = await supabase
                    .from('games')
                    .select('*')
