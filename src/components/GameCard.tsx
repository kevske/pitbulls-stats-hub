...
const ScoreProgressionChart: React.FC<{ game: GameStats }> = ({ game }) => {
...
const GameCard = memo(({ game, gameLogs, playerMap, onNavigate }: GameCardProps) => {
  // Sort top scorers for this game
  ...
  return (
    <Card
      ...
      onClick={() => onNavigate(`/games/${game.gameNumber}`)}
    >
            ...
            {(game.youtubeLink || (game.videoData && game.videoData.length > 0)) && (
              <div className="pt-3">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onNavigate(`/video-editor?game=${game.gameNumber}`);
                  }}
