...
const Games: React.FC = () => {
  ...
  // Optimize: Pre-calculate player map for O(1) lookup
  ...
  // Optimize: Pre-group game logs by game number for O(1) access
  const gameLogsByGame = useMemo(() => {
    const map: Record<number, PlayerGameLog[]> = {};
    gameLogs.forEach(log => {
      if (!map[log.gameNumber]) {
        map[log.gameNumber] = [];
      }
      map[log.gameNumber].push(log);
    });
    return map;
  }, [gameLogs]);
        ...
        <div className="space-y-4">
          {filteredGames.map((game) => (
            <GameCard
              key={game.gameNumber}
              game={game}
              gameLogs={gameLogsByGame[game.gameNumber] || []}
              playerMap={playerMap}
              onNavigate={navigate}
            />
          ))}
